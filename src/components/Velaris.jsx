...
export default function Velaris({
  ...
  useEffect(() => {
    ...
    // Honor reduced motion: freeze at t=0 instead of animating
    const reduceMotion =
    ...
    const createShader = (type, src) => {
      ...
      if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
        // Fail silently — hero falls back to CSS bg
    ...
    const render = (t) => {
      ...
      gl.uniform1f(locs.time, reduceMotion ? 0 : t * 0.001 * speed)
  ...
  // When height is "auto", container sizes to children — canvas absolutely fills it
  const isAuto = height === 'auto' || height === '100%'
  return (
    <div
      ref={containerRef}
      style={isAuto ? undefined : { height }}
      className={cn('relative w-full overflow-hidden', isAuto && 'h-auto', className)}
    >
      <canvas
        ref={canvasRef}
        aria-hidden="true"
