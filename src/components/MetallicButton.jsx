...
class MetallicShaderMount {
  constructor(parent, uniforms, speed) {
    this.gl = null;
    this.program = null;
    this.parent = parent;
...
export function MetallicButton({
  ...
  // Shell sizing: icon is square; text adapts with sleek card proportions
  const shellSize = isIcon
    ? size === "sm"
      ...
      : size === "lg"
        ...
        : "h-[38px] w-[38px]"
    : size === "sm"
      ? "h-[34px] px-3.5 min-w-[110px] w-auto"
      : size === "lg"
        ? "h-11.5 px-6 min-w-[142px] w-auto"
        : "h-[38px] px-[18px] min-w-[124px] w-auto";
  ...
  return (
    <div className={`relative inline-block ${className}`}>
      <div
        className="perspective-origin-[50%_50%] perspective-[1000px] perspective-1000 perspective-origin-center"
        style={{ perspective: 1000, perspectiveOrigin: "50% 50%" }}
      >
        <div
          className={`relative transform-3d inline-flex items-center justify-center ${shellSize}`}
          style={{ transformStyle: "preserve-3d" }}
        >
          {/* Layer 30: Text / Icon Layer */}
          <div
            className={`pointer-events-none relative z-30 flex items-center justify-center gap-2 transform-3d h-full w-full select-none`}
          ...
          >
            {isIcon ? (
              ...
              <span
                className={`text-sm font-normal whitespace-nowrap text-white [text-shadow:0px_1px_2px_rgba(0,0,0,0.5)] ${SETTLE} ${labelClassName}`}
              >
                {label}
              </span>
            )}
          ...
          {/* Layer 20: 3D Face with black-to-dark gradient & inset shadow on press */}
          <div
            className={`absolute inset-0 z-20 transform-3d ${pressShift} ${SETTLE}`}
            style={{ transform: "translateZ(10px)", transformStyle: "preserve-3d" }}
          >
            <div
              className={`absolute inset-[2px] rounded-full bg-[linear-gradient(180deg,#202020_0%,#000000_100%)] ${SHADOW_SHIFT} ${
                isPressed ? FACE_PRESSED : "shadow-none"
              }`}
            />
          </div>

          {/* Layer 10: 3D Rim with live WebGL2 liquid metallic shader */}
          <div
            className={`absolute inset-0 z-10 transform-3d ${pressShift} ${SETTLE}`}
            style={{ transform: "translateZ(0px)", transformStyle: "preserve-3d" }}
          >
            <div
              className={`rounded-full bg-transparent w-full h-full ${SHADOW_SHIFT} ${
                isPressed
                  ? RIM_PRESSED
                  : isHovered
                    ? RIM_HOVERED
            ...
            >
              <div
                ref={surfaceRef}
                className="metallic-button-canvas relative overflow-hidden rounded-full w-full h-full"
              />
            </div>
          </div>

          {/* Layer 40: Interactive button with ripples */}
          <button
            ref={buttonRef}
            type="button"
            onClick={handleClick}
            ...
            className={`absolute inset-0 z-40 cursor-pointer overflow-hidden rounded-full border-none bg-transparent outline-none transform-3d`}
